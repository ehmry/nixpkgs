...
# that a script resizes the filesystem at boot time.
{
  ...
  storePaths,
  # Whether or not to compress the resulting image with gzip
  compressImage ? false,
  zstd,
  # Enable f2fs compression by default
  featureList ? [
    "extra_attr"
    "compression"
  ],
...
let
  ...
  featureList' = lib.strings.concatStringsSep "," featureList;
in
pkgs.stdenv.mkDerivation {
  ...
  buildCommand = ''
    ...
    faketime -f "1970-01-01 00:00:01" fakeroot \
      mkfs.f2fs -U ${uuid} -l ${volumeLabel} -O "${featureList'}" -r -T 0 $img
