...
let
  fillMeta =
    { description ? nimbleMeta.description
  ...
  passthru = {
    inherit nimbleMeta;
    nimbleLdFlags = builtins.foldl'
      (list: pkg: list ++ pkg.passthru.nimbleLdFlags) nimbleLdFlags nimbleInputs;
    nimbleLdPath = builtins.foldl'
      (list: pkg: list ++ pkg.passthru.nimbleLdPath) nimbleLdPath nimbleInputs;
  };
in
stdenv.mkDerivation (attrs // {
  ...
  propagatedBuildInputs = builtins.foldl'
    (list: pkg: list ++ pkg.propagatedBuildInputs) nimbleInputs nimbleInputs;
